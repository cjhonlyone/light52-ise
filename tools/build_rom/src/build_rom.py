...
def main(argv):
    ...
    [vhdl_code,coe_str] = build_vhdl_code(params, xcode, top);
    ...
    try:
        ...
        fout.close()
        print "\nVHDL code table written to %s" % params['target']
    except:
    ...
    fout = None
    try:
        fout = open(params['target'].replace("obj_code_pkg.vhdl", "code.coe"), "w")
        fout.write(coe_str)
        fout.close()
        print "\ncoe table written to %s" %params['target'].replace("obj_code_pkg.vhdl", "code.coe")
    except:
        print "Trouble opening %s for output" % params['target'].replace("obj_code_pkg.vhdl", "code.coe")
    finally:
        if fout: fout.close()
